...
var _player_gmo_num: int = ProgressData.settings.optimize_gmo_num

# =========================== Extension =========================== #
func _get_unlocked_elements(player_index: int) -> Array:
    var unlocked: Array =._get_unlocked_elements(player_index)
    unlocked = _optimize_unlock_all_chars(unlocked, player_index)
    
    return unlocked

# =========================== Custom =========================== #
func _optimize_unlock_all_chars(unlocked: Array, player_index: int) -> Array:
    if ProgressData.settings.optimize_unlock_all_chars:
        var all_unlocked := []
